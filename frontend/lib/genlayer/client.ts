...
/**
 * Get the Ethereum provider — prefers MetaMask when multiple wallets are present.
 * Coinbase Wallet and other extensions inject themselves into window.ethereum.providers[]
 * which causes "Origin not allowed" errors if they win the race.
 */
export function getEthereumProvider(): EthereumProvider | null {
  ...
  // Multiple wallets inject into an array — pick MetaMask explicitly
  if (ethereum.providers?.length) {
    const metamask = ethereum.providers.find((p: any) => p.isMetaMask && !p.isCoinbaseWallet);
    if (metamask) return metamask;
  }

  // Single wallet — only use it if it's MetaMask
